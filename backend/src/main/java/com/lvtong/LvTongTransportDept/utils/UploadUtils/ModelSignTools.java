...
/**
 * 签名工具类
 * 与样例保持一致：TreeMap字典序，不过滤字段
 */
public class ModelSignTools {

    /**
     * 生成待签名字符串（与样例一致：TreeMap字典序）
     *
     * @param modelJsonStr JSON 格式字符串
     * @return key=value&key=value 格式字符串
     */
    public static String generateSignContent(String modelJsonStr) {
        // 使用 TreeMap 按字典序排列（与样例一致）
        Map<String, Object> map = JSON.parseObject(modelJsonStr, new TypeReference<TreeMap<String, Object>>() {});

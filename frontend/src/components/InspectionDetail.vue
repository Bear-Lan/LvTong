    ...
    <div class="detail-body">
        ...
        <div class="data-grid">
          ...
          <div class="data-col">
            <div class="data-row">
              ...
              <span class="data-value">{{ row.passcodeEnStationName || '-' }}</span>
            </div>
            <div class="data-row">
              <span class="data-label">出口站名称</span>
              <span class="data-value">{{ row.passcodeExStationName || '-' }}</span>
            </div>
            <div class="data-row">
      ...
      <div class="bottom-result-section" :class="{ 'is-success': (editable ? form.resultStatus : row.resultStatus) === 1, 'is-danger': (editable ? form.resultStatus : row.resultStatus) === 2 }">
        ...
        <div class="result-actions">
          <el-button v-if="canEdit" @click="visible = false">取消</el-button>
          <el-button v-if="canEdit" type="primary" @click="handleSubmit" :loading="submitting">保存修改</el-button>
          <el-button v-if="playbackReady" type="primary" @click="openPlayback">
            <el-icon><VideoPlay /></el-icon> 查看录像回放
          </el-button>
        </div>
      </div>
...
import { Plus, Picture, Edit, VideoPlay } from '@element-plus/icons-vue'
